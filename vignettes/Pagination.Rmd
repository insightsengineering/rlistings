...
vignette: >
  %\VignetteIndexEntry{Listing Pagination}
...
In many cases, listings more rows or more columns than fit on a single page. When this occurs, we may choose to use pagination to improve the readability of the listing and/or to fit in onto a standard-sized page for exporting or printing. When pagination is applied, all listing annotations (titles & footers) are printed on each of the returned page. Note that currently only mono-spaced fonts are supported for pagination.
...
In the R code below we will give a basic example of how to create an `rlistings` listing from a pre-processed data frame and paginate the listing using `paginate_listing`.
...
In the listing output above you can see that there are two key columns that we expect to be repeated on each page after pagination.
...
- Page type: letter (`page_type`)
- Font family: Courier (`font_family`)
- Font size: 12 (`font_size`)
- Line height: 1 (`line_height`)
- Landscape: false - portrait orientation (`landscape`)
- Margins: 0.5 inches for top/bottom margins, 0.75 inches for left/right margins (`margins`)

These as well as some additional parameters not listed here (see `?paginate_listing` for all options) can be set to fine-tune your pagination output.
...
In addition to specifying `page_type`, there are two alternative methods that can be used to specify page size when paginating a listing. If either of the two methods are implemented, the `page_type` argument will be ignored.

#### `pg_width` and `pg_height`
...
Considerations: 
- The `lpp` value do not include lines for titles and footers, which are included on every page. 
...
Considerations:
- If a column width smaller than the widest text in the column (or its label) is specified, then the width of that column will be set the number of characters of the widest text in that column. 
...
### Wrapping Title & Footer Information

In addition to setting page size, the user can also customize pagination by wrapping text in titles/footers to a given width. Titles and footers are wrapped to a specified width (in characters) using the `max_width` argument. If this parameter is not set then these annotations are automatically wrapped to the width of the table. If the `tf_wrap` argument is explicitly set to `FALSE` then titles & footers are printed as is.

For example, we can create a small listing: 

```{r}
lsting_tfwrap <- as_listing(
  df = adae[1:30, ],
  key_cols = c("ARMCD"),
  disp_cols = c("AGE", "SEX"),
  main_title = "This is a Very Long Title That Should Be Wrapped",
  main_footer = "This is a Very Long Footnote That Should Be Wrapped"
)

lsting_tfwrap
```

Then we apply pagination with `max_width` specified:

```{r}
paginate_listing(lsting_tfwrap, max_width = 15, tf_wrap = TRUE)
```
...
Similarly to `paginate_listing`, `export_as_txt` can also be used to paginate listing objects. Instead of returning a list of listings by page, the `export_as_txt` function will, if no file is specified, return a concatenated string value of all of the page content resulting after pagination. If the `file` parameter is specified, the `export_as_txt` function will instead write the result to the supplied `.txt` file.
...
Similarly to `paginate_listing` this creates a list of listings, where each list element corresponds to a new page and each page contains only one value of the given parameter. Note that the `page_prefix` argument can be specified to change the modify the text printed on each page to describe the current value of the specified parameter.
...
#### Combining Pagination by Parameter via `export_as_txt`
